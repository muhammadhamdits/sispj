...
class DetailItemController extends Controller
{
    ...
    public function get()
    {
        ...
        foreach($data->detailItem as $d){ 
            if($d->status == $data->kegiatan->program->urusan->periode->jenis){
                $max = $d->volume;
    ...
    public function min()
    {
        $id = $_GET['id'];
        $detailItem = DetailItem::findOrFail($id);
        $detailItem = DetailItem::where('created_at', '=', $detailItem->created_at)
                        ->where('status', '=', 0)
                        ->first();
        $min = 0;
        foreach($detailItem->detailKuitansi as $d){
            $min += $d->volume;
        }
        echo($min);
    }
